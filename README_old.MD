...
| **必須** | `date_start, date_end` | Object | - | 開始日〜終了日の指定 |
| **オプション** | `target_post_ids` | List[ID] | `null` | 特定のポストのみを狙い撃ちで精査する場合に指定 |
...
| **オプション** | `disp` | String | `none` | `none`: 全工程完遂<br>`targets`: 工程4まで（対象抽出の確認）<br>`results`: 工程9まで（DB反映直前までの確認） |
